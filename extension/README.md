...
1. 注册 host（免管理员；`rpa-core install-extension` 默认引导会自动注册）：
   ...
   ```
   写 host manifest 到浏览器认的位置：

   | 平台 | manifest 落盘位置 |
   |---|---|
   | Windows | `~/.rpa-core/native-host/com.rpa_core.ext_bridge.<browser>.json`，再由注册表 `HKCU\Software\<vendor>\<browser>\NativeMessagingHosts\com.rpa_core.ext_bridge` 指向它 |
   | macOS | **直接放浏览器目录**：`~/Library/Application Support/Google/Chrome/NativeMessagingHosts/`、`~/Library/Application Support/Microsoft Edge/NativeMessagingHosts/`（文件名 `com.rpa_core.ext_bridge.json`） |
   | Linux | `~/.config/{google-chrome,microsoft-edge}/NativeMessagingHosts/` |

   > macOS 上 Chrome 在 `Google/Chrome` **两层**下、Edge 是 `Microsoft Edge` **单层**——两平台
   > 路径表不同，照抄另一平台的会恒报「扩展离线」。
...
>
> **平台验证状态**：Windows 与 macOS 均已真机跑通（macOS 2026-09-19，Edge 153）；Linux 仅
> 单测/设计、未真机。逐部件状态表见 `docs/extension-install.md` §1.2。
...
**host 生命周期 = 扩展 port 生命周期**（ADR 0015）：stdin EOF（扩展断开 / 浏览器退出）即清理
端点并退出。所以判定「扩展在线」**不需要心跳**——「端点可连接」就等价于「扩展在线」，
`status` 也不含心跳字段。排查在线问题时不必去找 15s ping：那是
`.harness/spike/native_messaging/` 里 **spike 扩展**的观测手段；产品扩展只在收到 host 的
`ping` 时回 `pong`，自身从不主动发。

> 2026-09-19 macOS 实测：host 单进程连续服务 **11 小时 23 分**，端点 socket 自 bind 起未重建、
> 日志零断连事件；扩展 reload 与浏览器完全退出则立即回收 host 并删除端点文件。
...
（没有"启动/切换浏览器"概念）。随 playwright 一并移除的是 `transport` / `channel`
**入参**——注意 `browser.navigate` **命令本身仍在命令面中**（`rpa-core catalog` 可查，
入参为 `browserType` / `action` / `url` / `timeoutMs` / `onTimeout` / `commandLineArgs`，
`additionalProperties: false`），只是它不再暴露"走哪个传输通道"这一项：现在唯一通道就是本扩展。

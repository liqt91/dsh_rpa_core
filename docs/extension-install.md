...
> 状态：代码与合同测试完成（2026-09-06）；静默安装机制本机实测通过，产品主路径为商店上架；本机另有策略键清除工具（§6）。**Chrome 152 + Edge 152(.62) 双浏览器对照实测（2026-09-07，含影刀插件真机重装对比）见 §6.5**——外部注册表来源是否放行由 manifest `update_url` 归属决定：Chrome 指向 CWS、Edge 指向 CWS 均可点启用（无需上架）；Edge 的 update_url 指向自家商店 URL（未上架）则报"损坏"。manifest `update_url` 已于 09-07 改为 CWS。
...
- 本机实证：全新 profile Edge/Chrome 启动后扩展安装成功（`location=3` EXTERNAL_REGISTRY），
  零交互零 UAC 零网络。**未上架期间在双浏览器均可用但非零点击**：manifest `update_url` 指向 CWS 时
  用户需点一次"启用"确认（Chrome 152/Edge .62 实测，§6.5）；§6.4 曾记录 Edge .53 消费版"未知来源"
  硬禁用（启用开关灰色），该现象在 .62 未复现（差异待统一条件复核）。
...
- **本地 CRX 静默路线（外部注册表）——09-07 修订**：Chrome 152 与 Edge 152(.62) 上 manifest `update_url` 指向 **CWS** 时，安装后仅需用户点一次"启用"确认即生效（§6.5），未上架期间即可用，但无法零点击。注意 **Edge 的 update_url 不要指向自家 Edge 商店 URL**（未上架 CRX 会报"损坏"）。§6.4 记录的 .53 消费版"未知来源灰禁"行为未在 .62 复现（差异待统一条件复核）。代码保留（合同测试覆盖），不预置 HMAC 确认记录（影刀式灰手法，不复刻）。

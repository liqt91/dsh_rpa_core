...
状态：初版（2026-09-20，维护者已评审并定案关键取舍）；**L1 浏览器通道已落地**（M38，2026-09-22——32 条命令 / 180 个变体，见 `.harness/tasks/M38-command-matrix.md`）
...
| **按需：指令覆盖（执行层）** | L1 全量参数矩阵（跑变体） | `RPA_COMMAND_MATRIX=1 pytest tests/commands`（缺省不收集，见 `tests/commands/conftest.py`） |

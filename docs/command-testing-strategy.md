...
3. **L2 浏览器冒烟**（本地 HTTP 测试页 + 真扩展 + **与 L1 共用同一份 JSON 用例表**）——
   **进行中（M38 S4，2026-09-23）**：S4.1 最小链路（真扩展 + 隔离 profile + 端点前缀隔离）
   与 S4.2 读族 6 命令 15 变体已真机跑通；「同一份表」的落地形态是变体级 `l2` 块
   （`page` + `inputs` 覆盖 + 真机 `expect`，L1 的 `expect` 不动）。详见 M38 任务单 §1.8/§1.9

...
状态：初版（2026-09-20，维护者已评审并定案关键取舍）；**L1 契约矩阵三条通道已全部建表并跑通
执行层**（M38，2026-09-22——浏览器 32 / 数据+工作流 18 / 桌面 36，共 **86 条命令 / 567 个变体**，
见 `.harness/tasks/M38-command-matrix.md`）；桌面那 36 条在补完靶子后**已在真机上跑过**并逐类校正
（**190 变体 → 190 passed / exit 0**，两个曾用 xfail 钉住的实现缺口已在 S2.3 + 尾巴 #1 修完，
见 §2「落地进展」与 M38 任务单 §1.6、§1.14）
...
其契约测试进默认门禁）。桌面 **36 条也已建表**（M38 S2，2026-09-22）：`cases/desktop.json`（UIA 17 条；
建表时 78 变体，补靶子后 80）+ `cases/desktop_win32.json`（Win32 19 条；81 → 92 变体），驱动与装配是
...
**86 条命令全部有表**。
**执行层已跑（2026-09-22，S2.2）**：先按维护者指令「先补靶子」给靶子补上原生菜单栏（`MainMenu`，
**不是** `MenuStrip`——托管控件画在客户区里、`GetMenu(hwnd)` 拿不到，而 win32 的菜单路径走的正是
原生菜单）、ListBox/ComboBox、可拖 Label、只读 Edit（两后端各得一个文本恒定的元素锚点），再把
**159 个只建表没跑过的变体第一次上真机**——30 处「读代码猜的期望」被实测逐类推翻并校正
（空会话串其实是**回退默认会话**、整串占位符必须保留原类型、`getWindowList` 的声明说要会话实现也
要、win32 无效句柄报 `EXECUTOR_FAILED`），两条旧判因当场被推翻（win32 的 `title` 其实可用、
`menuSelect` 的正路径真机通过）。最终 **172 个变体 → 170 passed + 2 xfailed / exit 0**
（两条 xfail 是 `screenshot` 的 Pillow 依赖缺口，用新增的**变体级 `knownGap`** 钉成严格 xfail：
缺口一修就 XPASS 转红）。完整记录见 M38 任务单 §1.6。
**该两个缺口已修完**（S2.3 加 Pillow + 尾巴 #1 实现 win32 原生消息），且桌面两表在修复过程中
**又长了变体**（UIA 80→85、Win32 92→105）——**当前真值是 190，实测 `190 passed / exit 0`**
（2026-09-23 一手重跑；上文的 172 是当日数字，见 M38 任务单 §1.14 的订正表）。
...
- ~~**win32 后端要另做靶子**~~ **（2026-09-22 纠正：这句不成立）** 原文写的是「它的定位器认
...
### 4.0 工作台「指令测试」页签覆盖到哪一层（**2026-09-23 如实记账**）

页签（M38 S1.2，`gui/command_matrix.py`）**只点火 L1**，这是刻意的，不是遗漏：

| 层 | 页签能跑吗 | 为什么 |
|---|---|---|
| 浏览器 / 数据 / 工作流 **L1** | ✅ 主按钮 | 268 个变体，假扩展，零副作用面 |
| 桌面 UIA + Win32 **L1** | ❌ 会被收集但**全 skip** | 驱动要求 `RPA_DESKTOP_E2E=1`（真开窗 + 抢前台）；页签不设该变量 |
| 浏览器 **L2** | ❌ **连收集都不收集** | 归 `RPA_BROWSER_L2` 管；且它**与 `RPA_COMMAND_MATRIX` 不叠加**——只开 L2 时收集层会 ignore 掉全部 L1 驱动（`tests/commands/conftest.py` 末段） |
| 覆盖率静态校验 | ✅ 独立按钮 | 就是默认门禁那一步 |

页面上的 `scope_label` 把这三笔账**写在界面上**（`ScopeBreakdown` 纯函数按「文件名 + 变体是否带
`l2` 块」算，与收集口径同源、不写死数字），免得「已覆盖：…」被读成「点下去这些都会跑」。

**把桌面与 L2 接进页签**是下一个切片（三个明确目标：L1 / L1+桌面 / L1+L2），
前置是改收集口径让两个开关可叠加——**记在 M38 任务单 §6，尚未开工**。
...
2. **L1 桌面命令用例表**（UIA / Win32：参数整形 + 错误分类）——**已建表并跑通执行层
   （M38 S2 + S2.2，2026-09-22）**：两表 + 两驱动 + 共享装配，`PENDING_NAMESPACES` 清空；
   瞄准靶子缺失的可测面先补齐（原生菜单栏 / ListBox / ComboBox / 可拖 Label / 只读 Edit），
   172 个变体 → 170 passed + 2 xfailed，见 §2 落地进展与 M38 任务单 §1.6。
   **当前真值 190 变体 → 190 passed / exit 0**（2026-09-23 随 S2.3 + 尾巴 #1 加变体并转正两条
   xfail 后重跑，见 M38 任务单 §1.14）
3. **L2 浏览器冒烟**（本地 HTTP 测试页 + 真扩展 + **与 L1 共用同一份 JSON 用例表**）——
   **已落地（M38 S4，2026-09-23）**：变体级 `l2` 块（`page`/`inputs`/`expect`/`pre`/`verify`/
   `session`），真扩展 + 隔离 profile + 端点前缀隔离；浏览器 32 条命令全部有了 L2 处置
   （109 个 l2 块，真机 110 passed），过程中修复 3 个真机才可见的产品 bug
   （check 三操作反转 / goBack API 不可靠 / timeoutMs 不进 args）。详见 M38 任务单 §1.8–§1.12
4. **L2 桌面对齐**（复用 WinForms fixture / 记事本切片；显式开关内跑）——**事实上已由
   S2 定案超越**：桌面通道的 L1 载体就是真桌面 fixture（非打桩绑定层），即桌面侧的
  「契约 + 真机」在 L1 一层同时成立，无需独立 L2
5. L3 / L4 维持按需与人工

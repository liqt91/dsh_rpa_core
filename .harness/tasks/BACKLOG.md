...
- [ ] **M27 工作台（首页）+ 编辑器两段式**（`active`，ADR 0017）
  - 计划：`M27-workbench-home.md`；决策：ADR 0017（独立工作台窗口；运行控制留在编辑器；
    删除流程提示后连同元素资产/数据表格/运行历史一并删除；多窗口编辑器推迟）
  - 目标：流程管理（列表/新建/复制/重命名/删除/导入导出 + 运行入口 + 状态）与全局运行历史
    （可筛选、双击打开编辑器看时间线）搬到独立首页窗口；编辑器保持现有单窗口
  - 切片：S1 工作台骨架（流程列表 + 打开/新建）→ S2 运行历史迁入（全局 + 筛选）→
    S3 管理动作（含连带删除与确认）→ S4 运行入口/状态联动 + 测试文档

## 后续任务

- [ ] **M26 流程 inputs 声明编辑 UI**（`planned`——原为 M25 之后的第一项，因维护者定向做工作台（M27，ADR 0017）而顺延；计划见 `M26-flow-inputs-editor.md`）

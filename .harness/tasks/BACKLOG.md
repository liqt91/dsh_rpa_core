...
- [ ] M11 编辑器交互升级：结构化树形画布（`active`）
  - 计划：`M11-editor-tree.md`
  - 完成门槛：切片 1（树模型 + 树形渲染 + DnD v2）后维护者确认拖拽体验，再完成表单/多选/复制粘贴/快照撤销；零构建边界不破；full gate。

## 后续任务

- [ ] M10 元素捕获：桌面 UIA hit-test + 浏览器双通道，含 S1 验证（`planned`，计划 `M10-capture.md`；S1 已完成，余实装）

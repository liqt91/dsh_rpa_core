...
- 浏览器捕获 live 复用登录态 → **扩展路线为主**；非扩展备选 = 专用持久 profile（自动化浏览器内登录一次）。
- 桌面捕获走 UIA hit-test，不涉及上述限制。

## 任务

- [ ] 编写 ADR 0007：dev server 定位（设计期工具，不承载 run）、新顶层包 `rpa_core.devserver` 与架构边界（只读复用 catalog/编译器，绝不共享运行进程）、workflow 文件目录约定（受限根目录）、浏览器捕获传输契约（扩展为主 / 持久 profile 为辅 / CDP attach 默认 profile 已排除的实测依据）、桌面捕获契约
...
- [ ] 捕获端点骨架：`POST /api/capture/desktop/start|pick|cancel`（UIA hit-test）与 `POST /api/capture/browser/*`（M10 实装，本里程碑先定义契约与 501 占位）
